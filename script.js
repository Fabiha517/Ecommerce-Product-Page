...
<img class="deletesvg" src="images/icon-delete.svg" alt="">
...
function allowImageOverlay() {
	const productImages = document.querySelector(".productImages");
	if (meqs.matches) {
		// Attach click listener only once for desktop
		if (!overlayListenerAttached) {
			productImages.style.pointerEvents = "auto";
			overlayListenerAttached = true;
		}
	} else {
		// Remove listener when on mobile
		if (overlayListenerAttached) {
			productImages.style.pointerEvents = "none";
			overlayListenerAttached = false;
		}

		// Also remove overlay if it's currently open
		const openOverlay = document.querySelector(".image-wrapper");
		if (openOverlay) {
			openOverlay.remove();
		}
	}
...
allowImageOverlay();
meqs.addEventListener("change", allowImageOverlay);
...
// const deletesvg=document.querySelector(".deletesvg")
